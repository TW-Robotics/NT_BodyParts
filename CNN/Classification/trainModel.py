...
for i in range(0,len(Class_Names)):
    ...
    plt.close('all')
    #--- store as npy files ---#
    np.save(Class_Names[i]+"_grad.npy", grads)
    np.save(Class_Names[i]+"_LRP_10.npy", LRP_10_i[0,:,:,0])
    np.save(Class_Names[i]+".npy", test_data[0][index])
#-------------------#
#--- All of them ---#
#-------------------#
for i in range(0,prediction.shape[0]):
    ...
    pred_row = prediction[i,:]
    index = np.argmax(pred_row)
    prefix=os.path.splitext(test_data[2][i])[0].split("/")[-1] + "_"
    ...
    plt.close('all')
    #--- store as npy files ---#
    #np.save(str(i)+'_'+prefix+Class_Names[np.argmax(ground_truth[i,:])]+"_grad.npy",grads)
    #np.save(str(i)+'_'+prefix+Class_Names[np.argmax(ground_truth[i,:])]+"_LRP_10.npy",LRP_10_i[0,:,:,0])
    #np.save(str(i)+'_'+prefix+Class_Names[np.argmax(ground_truth[i,:])]+".npy", test_data[0][i])
    np.save(prefix+"_grad.npy",grads)
    np.save(prefix+"_LRP_10.npy",LRP_10_i[0,:,:,0])
    np.save(prefix+".npy", test_data[0][i])
