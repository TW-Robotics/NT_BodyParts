...
#replace.data <- suppressMessages(as.matrix(read_table2("../../dataGenerator/kFoldData/replace_20205809_085859.csv", col_names = FALSE)))
#Name, Path, nr. features, procrustes flag


HMC.BGPLVM.noReplace.manual      <- c("BGPLVM_NoReplace",         "./resdata/",     14,!is.ProcrustesLandmark, !with.replace)
HMC.Procrustes.noReplace.manual  <- c("Procrustes_NoReplace",  "./resdata/",     14, is.ProcrustesLandmark, !with.replace)
#--- Create list to process ---#
processor <- rbind( HMC.BGPLVM.noReplace.manual,
                    HMC.Procrustes.noReplace.manual
                    )
#---------------------------#
#--- (De)activate ggplot ---#
#---------------------------#
...
for(i in seq(1,nrow(processor))){
  ...
  for(n in tries){
    rel.local <- suppressMessages(as.matrix(read_csv(paste(processor[i,2],processor[i,1],"/",n,"_Relevance.csv",sep = ""))))
    #------------------------------------#
    #--- Combine LS Procrustes values ---#
    #------------------------------------#
    if(as.logical(processor[i,4])){
      cat("Convert Procrustes LS vals\n")
      rel.local <- rel.local[1:28]
      rel.memory <- c()
      for(k in seq(1,length(rel.local),2)){
        rel.memory <- c(rel.memory, sqrt(rel.local[k]^2+rel.local[k+1]^2))
      }
      memory.relevance[,n+1] <- rel.memory
    }else{
      #------------------------------------------------------------#
      #--- GP-LVM relevance data can be used without conversion ---#
      #------------------------------------------------------------#
      memory.relevance[,n+1] <- rel.local[1:14]
    }
  }
  ...
  for(k in seq(1,top.N)){
    ...
    #--- Prepare plot ---#
    #--------------------#
    pdf(paste(processor[i,1],"_R",k,"_pie_HMC.pdf",sep = ""))
    #if(ggplot.piePlot){
    #  par(family='serif') 
    #  par(mfrow = c(top.N,1), 
    ...
    for(m in index.notZero){
      ...
      if(as.logical(processor[i,4])){   #Check if Procrustes features
        ...
        if(TRUE){#grepl("manual",processor[i,1])){
          #--- Manually selected ---#
    ...
    system(paste("pdfjam ",  
                 processor[i,1],"_R",1,"_pie_HMC.pdf ",
                 processor[i,1],"_R",2,"_pie_HMC.pdf ",
                 processor[i,1],"_R",3,"_pie_HMC.pdf ",
                 processor[i,1],"_R",4,"_pie_HMC.pdf ",
                 processor[i,1],"_R",5,"_pie_HMC.pdf ",
                 "--nup 5x1 --landscape --outfile ",processor[i,1],"_pie_HMC.pdf",sep = ""))
  system(paste("pdfcrop --margins '0 0 0 0' --clip ",processor[i,1],"_pie_HMC.pdf"," ",processor[i,1],"_pie_HMC.pdf",sep = ""))
  system("rm *_R*_pie_HMC.pdf") #Remove old results
  #break
}
system("mkdir HMCData")
system("mv *ranking*csv HMCData/.")